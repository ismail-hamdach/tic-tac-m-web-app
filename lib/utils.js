...
export const formatTimeTo12Hour = (timeString, showSeconds = false) => {
  const [hours, minutes, seconds] = timeString.split(':').map(Number);
  const period = hours >= 12 ? 'PM' : 'AM';
  const formattedHours = hours % 12 || 12; // Convert to 12-hour format
  return showSeconds ? `${formattedHours}:${String(minutes).padStart(2, '0')}:${String(seconds).padStart(2, '0')} ${period}` : `${formattedHours}:${String(minutes).padStart(2, '0')} ${period}`;
}
...
export const translateText = (title) => {
  // Retrieve the language from the URL
  const pathname = window.location.pathname; // Get the current pathname
  const langMatch = pathname.match(/^\/(en|fr|ar)/); // Match supported languages
  const lang = langMatch ? langMatch[0]?.slice(1) : 'fr'; // Default to 'fr' if not found

  // const trans = getDictionary(lang);
  return translate(title, trans); // Added return statement to return the translated title
}